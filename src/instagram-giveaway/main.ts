...
function userNameOccurrences(dirName: string, fileName: string, uniqueNamesHash: Dictionary<number>): Promise<void> {
  return new Promise((resolve) => {
...
export function processGiveAwayResults(): void {
  ...
  readdir(DIR_PATH_2MIL, async (err: ErrnoException | null, innerFiles: string[]) => {
    ...
    const uniqueHash: Dictionary<number> = getHashTable(allFilesContent);
...
function uniqueValues(userNamesHash: Dictionary<number>): void {
...
function existInAllFiles(userNamesHash: Dictionary<number>): void {
...
function existInAtLeastTen(userNamesHash: Dictionary<number>): void {
